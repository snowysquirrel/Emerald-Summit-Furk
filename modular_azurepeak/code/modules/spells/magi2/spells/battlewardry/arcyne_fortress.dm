// Arcyne Fortress — Battlewardry Mastery (T4 additive variant). Port of Azure-Peak
// arcyne_fortress.dm. Raises a 5x5 perimeter of directional arrow wards around the caster.
// Uses our ported arrow_ward_magi2 structure (new(turf, caster, duration) + set_shield_dir).

/datum/action/cooldown/spell/arcyne_fortress_magi2
...
/datum/action/cooldown/spell/arcyne_fortress_magi2/cast(atom/cast_on)
	. = ..()
	...
	for(var/list/entry in perimeter_data)
		...
		var/obj/structure/arrow_ward_magi2/wall = new(T, H, cooldown_time)
