// Iron Skin — Ferramancy self/ally buff. Casting on another mob extends the duration.
// click_to_activate + charge_then_click upstream — for the pilot we keep click_to_activate
// only, no charge_then_click two-tap (that input flow isn't ported). Player clicks the
// action button, then clicks the target — single click target after charge completes.

/datum/action/cooldown/spell/iron_skin_magi2
	name = "Iron Skin"
	desc = "Conjure bits of arcyne iron and steel to surround the target's armor, blunting incoming blows and protecting their equipment. \
...
/datum/action/cooldown/spell/iron_skin_magi2/cast(atom/cast_on)
	. = ..()
