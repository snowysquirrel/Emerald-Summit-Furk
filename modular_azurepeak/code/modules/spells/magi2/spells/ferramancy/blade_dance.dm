// Blade Dance — Ferramancy Mastery (T4 additive variant). Port of Azure-Peak blade_dance.dm.
// Conjures a stationary 7x7 storm of spectral blades that slashes everything inside while the
// caster stands still. Uses our ported spinning_dagger_magi2 visual.
...
/datum/action/cooldown/spell/blade_dance_magi2
...
/datum/action/cooldown/spell/blade_dance_magi2/cast(atom/cast_on)
	. = ..()
...
/datum/action/cooldown/spell/blade_dance_magi2/proc/caster_moved()
	SIGNAL_HANDLER
	end_dance()

/datum/action/cooldown/spell/blade_dance_magi2/proc/caster_interrupted()
	SIGNAL_HANDLER
	end_dance()

/datum/action/cooldown/spell/blade_dance_magi2/proc/end_dance()
	if(owner)
...
/obj/effect/blade_dance_zone
	...
	var/datum/action/cooldown/spell/blade_dance_magi2/source_spell
	var/ticks_remaining
...
/obj/effect/blade_dance_zone/Initialize(mapload, mob/living/carbon/human/summoner, datum/action/cooldown/spell/blade_dance_magi2/spell_ref)
	. = ..()
	caster = summoner
...
/obj/effect/blade_dance_zone/proc/setup_visuals()
	...
	var/expand_time = BLADE_DANCE_WINDUP // already in deciseconds (ticks) for the expansion
	for(var/turf/T in range(effect_radius, center))
		if(T == center)
			continue
		var/obj/effect/temp_visual/spinning_dagger_magi2/D = new(center, BLADE_DANCE_DURATION + BLADE_DANCE_WINDUP + 2 SECONDS, FALSE)
		blade_visuals += D
		var/dx = (T.x - center.x) * 32
		var/dy = (T.y - center.y) * 32
		animate(D, pixel_x = dx, pixel_y = dy, time = expand_time, easing = EASE_OUT)
		addtimer(CALLBACK(D, TYPE_PROC_REF(/obj/effect/temp_visual/spinning_dagger_magi2, start_spinning)), expand_time)
