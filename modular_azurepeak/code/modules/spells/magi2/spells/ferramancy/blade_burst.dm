// Blade Burst — Ferramancy ground-targeted AOE. Telegraph then blades cut from below.
...
/datum/action/cooldown/spell/blade_burst_magi2
	name = "Blade Burst"
	desc = "Summon a storm of arcyne blades erupting from the ground in an area. \
		After a short delay, the blades burst upward and cut anything still standing in the zone. \
...
/datum/action/cooldown/spell/blade_burst_magi2/cast(atom/cast_on)
	. = ..()
	...
	for(var/turf/affected_turf in get_hear(area_of_effect, T))
		if(!(affected_turf in get_hear(cast_range, source_turf)))
			continue
		new /obj/effect/temp_visual/trap/ferramancy_magi2(affected_turf)
...
/datum/action/cooldown/spell/blade_burst_magi2/proc/blade_burst_detonate(turf/T, turf/source_turf, mob/living/carbon/human/caster)
		...
		for(var/mob/living/L in affected_turf.contents)
			...
			if(ishuman(caster) && ishuman(L))
				...
				arcyne_strike(caster, L, null, total_damage, target_zone, \
					...
					damage_type = BRUTE, npc_simple_damage_mult = npc_damage_mult, \
					skip_animation = TRUE)
			else
