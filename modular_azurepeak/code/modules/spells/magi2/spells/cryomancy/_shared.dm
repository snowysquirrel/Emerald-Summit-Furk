...
// frosted3 (-2 SPD, 1.2x action CD). A fourth stack bursts for 25 BURN and clears all frost.
// Fire spells shatter frost stacks (handled in fire spell on_hit procs).
// Reuses existing /obj/effect/temp_visual/snap_freeze and /obj/effect/temp_visual/trapice
// from code/modules/spells/spell_types/wizard/invoked_aoe/snap_freeze.dm — no redefine.
...
/// Apply one frost stack to the target. Escalates frosted1 -> frosted2 -> frosted3 -> burst (25 burn, clears frost).
...
/// Decrement one frost stack from the target. Used by fire spells to shatter frost.
