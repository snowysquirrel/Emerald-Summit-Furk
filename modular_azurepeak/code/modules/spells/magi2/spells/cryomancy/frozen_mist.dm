// Frozen Mist — Cryomancy Mastery (T4 additive variant). Port of Azure-Peak frozen_mist.dm.
// Conjures a lingering 5x5 freezing cloud: full frost stacks + burn damage per second,
// extinguishes fires/hotspots inside. Uses our ported frost-stack helpers.

/datum/action/cooldown/spell/frozen_mist_magi2
...
/datum/action/cooldown/spell/frozen_mist_magi2/cast(atom/cast_on)
	. = ..()
	...
	new /obj/effect/frozen_mist_magi2(centerpoint, H, src)
	return TRUE

/obj/effect/frozen_mist_magi2
	name = "frozen mist"
...
/obj/effect/frozen_mist_magi2/Initialize(mapload, mob/living/summoner, datum/action/cooldown/spell/frozen_mist_magi2/spell_ref)
	. = ..()
	caster = summoner
	source_spell = spell_ref

	var/this_duration = 10 SECONDS
	if(istype(spell_ref))
		this_duration = spell_ref.mist_duration
		tick_damage = spell_ref.tick_damage
		effect_radius = spell_ref.mist_radius

	ticks_remaining = this_duration / (1 SECONDS)

	var/turf/center = get_turf(src)
	for(var/turf/T in range(effect_radius, center))
		var/obj/effect/temp_visual/frozen_mist_tile/V = new(T, this_duration + 2 SECONDS)
		mist_visuals += V

	playsound(src, 'sound/spellbooks/crystal.ogg', 80, TRUE)
	initial_frost(center)
	START_PROCESSING(SSprocessing, src)
	QDEL_IN(src, this_duration + 1 SECONDS)

/obj/effect/frozen_mist_magi2/proc/initial_frost(turf/center)
	for(var/turf/T in range(effect_radius, center))
...
/obj/effect/frozen_mist_magi2/Destroy()
	caster = null
	source_spell = null
...
/obj/effect/frozen_mist_magi2/process(delta_time)
