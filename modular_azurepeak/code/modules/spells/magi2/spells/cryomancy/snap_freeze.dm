// Snap Freeze — Cryomancy ground-targeted delayed AOE.
// MVP config: click_to_activate FALSE — centers on the tile a few squares in front of caster
// (parallels Fire Curtain). Upstream uses click-to-target.
...
/datum/action/cooldown/spell/snap_freeze_magi2/cast(atom/cast_on)
	. = ..()
	...
	if(click_to_activate)
		...
		T = get_ranged_target_turf(H, H.dir, 3)
	if(!T)
		return FALSE
...
/datum/action/cooldown/spell/snap_freeze_magi2/proc/snap_freeze_detonate(turf/T, turf/source_turf, mob/living/carbon/human/caster)
