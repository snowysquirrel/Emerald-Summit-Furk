...
//Point the bombard at whatever it's currently aimed to fire on. The heavy bombard's sprite
//(bombard48.dmi "bigmortar") carries four directional frames, so snap to the dominant cardinal axis
//of the target. Harmless on single-direction sprites like the portable bombard's smallmortar.
/obj/structure/bombard/proc/update_facing()
	if(!xinput && !yinput)
		return
	var/dx = (xinput + xdial) - x
	var/dy = (yinput + ydial) - y
	if(!dx && !dy)
		return
	if(abs(dx) >= abs(dy))
		setDir(dx > 0 ? EAST : WEST)
	else
		setDir(dy > 0 ? NORTH : SOUTH)

/obj/structure/bombard/examine(mob/user)
	. = ..()
	if(HAS_TRAIT(user, TRAIT_FUSILIER))
		//Re-obfuscate the stored target so the readout matches the LIP values the operator entered
		//(xinput/yinput are stored deobfuscated). 0/0 means it hasn't been aimed yet.
		var/disp_x = (xinput || yinput) ? obfuscate_x(xinput) : 0
		var/disp_y = (xinput || yinput) ? obfuscate_y(yinput) : 0
		. += "...<br>\
			<small>Target 'X-LIP': <span class='warning'>[disp_x]</span> <br>\
			Target 'Y-LIP': <span class='warning'>[disp_y]</span> <br>\
			'X-LIP' Fine-Dial (-10 to 10): <span class='warning'>[xdial]</span> <br>\
			'Y-LIP' Fine-Dial (-10 to 10): <span class='warning'>[ydial]</span> <br>\
			<br>\
...
//Right-clicking the bombard with a palantir in hand loads its stored coordinates straight in.
/obj/structure/bombard/attack_right(mob/user)
	var/obj/item/rogueweapon/palantir/P = user.get_active_held_item()
	if(istype(P))
		P.load_bombard(src, user)
		return TRUE
	return ..()
...
/*		if(T.arcyne_shroud_check())//Has the magos warded the area? Some locations are protected by default, such as his tower...
		...
		if(do_after(user, 30, src))
			...
			if(zdial == null)//No offset? Somehow? That's fine! We do the bombard's Z!
				...
				zdial = temp_targ_z
			update_facing()//Turn the barrel to face the new target.
		else
			busy = 0
	...
	if (choice == "Dial")
		...
		if(do_after(user, 15, src))
			...
			xdial = temp_dial_x
			ydial = temp_dial_y
			update_facing()//A dial nudge can shift the aim - re-face to match.
		else
			busy = 0
		...
		if(do_after(user, 3 SECONDS, src))
			...
			spawn(travel_time) //What goes up
				...
				spawn(45) //Must go down
					if(!QDELETED(cannonball))//A lit incendiary may have already cooked off mid-flight.
			...
			message_admins(msg)
			log_admin(msg)
			//Keep the bombard dialed in after firing - coords/dials persist so it can re-fire at the
			//same target. Only the powder load is consumed each shot (re-fill and re-ram to fire again).
			powder = FALSE
			rammed = FALSE
