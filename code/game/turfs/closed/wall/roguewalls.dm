...
/turf/closed/wall/mineral/rogue/examine(mob/user)
	. += ..()
	if(max_integrity)
		if(can_show_exact_integrity(user))
			// A carpenter/mason/etc. reads the exact integrity of a wall they could build.
			. += span_notice("Integrity: [turf_integrity]/[max_integrity]")
		else
