// Ported from RW1710: contraption gloves (voltic gauntlets).
...
/obj/item/clothing/gloves/roguetown/contraption/voltic/examine(mob/user)
	. = ..()
	. += span_info("Insert a cog into the gauntlets to charge them.")
...
/obj/item/clothing/gloves/roguetown/contraption/voltic/attack_right(mob/user)
	...
	if(cooldowny && world.time < cooldowny + cdtime)
		to_chat(user, span_warning("The gauntlets' machinery is still recalibrating. [DisplayTimeText(cooldowny + cdtime - world.time, 1)] remains."))
		return
